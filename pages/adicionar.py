...
duracao = st.number_input("Duração da coleta (dias úteis):", step=1)
amostra = st.number_input("Amostra:", step=1)
...
media = st.number_input("Digite a média de entrevistas por dia:")
hc_sugerido = round(amostra / duracao / media)
...
for semana in projeto.semanas:
    ...
    for i, dia in enumerate(semana.dias_uteis):
        if dia.data >= projeto.data_inicio_coleta and dia.data <= projeto.data_fim:
            with colunas[i]:
                hc = st.number_input("HC - " + dia.data.strftime("%d/%m/%Y"), step=1, value=hc_sugerido)
                dia.hc = hc
                entrevistas_semana += hc

    produtividade = st.number_input(f"Produtividade da semana {num_semana}")
